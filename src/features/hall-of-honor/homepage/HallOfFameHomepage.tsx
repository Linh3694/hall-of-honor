// @ts-nocheck — refactor dần sang kiểu đầy đủ (file animation + i18n object lớn)
...
import { useScrollDrivenStyle } from "@/shared/hooks/useScrollDrivenStyle";
...
import {
  ...
  HOMEPAGE_SWIPER_SPEED_MS,
} from "@/core/config";
import { HallOfHonorHeader } from "../components/layout/HallOfHonorHeader";

/**
 * Carousel: Splide dùng cho marquee/ticker (auto-scroll); Swiper cho slider có điều khiển (đội ngũ).
 * Gộp 1 thư viện đồng nghĩa refactor lớn — giữ tách vai trò để tránh regression UI.
 */

/** Link mặc định vào chi tiết “Học bổng Tài năng” khi người dùng ở homepage (AISU / Wellspring) */
const DEFAULT_SCHOLARSHIP_TALENT_DETAIL_PATH =
  "/detail/scholarship-talent/ai-su-wellspring";

/** Giá trị đếm thống kê trên homepage (đồng bộ với design cũ) */
const HOMEPAGE_STAT_COUNTS = {
  categories: 11,
  students: 600,
  classes: 12,
};
...
const HallofFame = () => {
  ...
  const ReadMoreText = ({ text }) => {
    ...
    const threshold = isMobileResponsive ? DESCRIPTION_TRUNCATE_CHARS : Infinity;
  ...
  const principals = [
    {
      name: t("principal1.name", { ns: "homepage" }),
      title: t("principal1.title", { ns: "homepage" }),
      message: t("principal1.message", { ns: "homepage" }),
      quote: {
        text: t("principal1.quote.text", { ns: "homepage" }),
        author: t("principal1.quote.author", { ns: "homepage" }),
      },
    ...
    // },
    {
      name: t("principal3.name", { ns: "homepage" }),
      title: t("principal3.title", { ns: "homepage" }),
      message: t("principal3.message", { ns: "homepage" }),
  ...
  useGsapSectionReveal(section2Ref);

  const { textOpacity, textTransform } = useScrollDrivenStyle();
  ...
  }, [textOpacity, shouldStartCounting]);
  ...
  const starStudents = t("starStudents", { returnObjects: true, ns: "homepage" });
  ...
  return (
    <>
      <HallOfHonorHeader
        ...
        hallHonorLabel={t("hallhonor")}
        onHallHonorClick={() =>
          navigate(DEFAULT_SCHOLARSHIP_TALENT_DETAIL_PATH)
        }
      />

      <div className="relative w-full flex flex-col gap-0">
        {/* Section 1: Video — section phải relative để inset-0 neo đúng hero (trước đây neo nhầm lên wrapper cả trang → không thấy clip) */}
        <section
          ref={section1Ref}
          className="relative z-0 mb-0 w-full h-[100svh] min-h-[100svh] overflow-hidden bg-black"
        >
          {/* object-cover: lấp đầy khung — object-contain để lộ letterbox đen (#000) của <video>, lệch với nền → vệt giữa hai section */}
          <video
            aria-hidden
            className="pointer-events-none hidden lg:block absolute inset-0 h-full w-full object-cover object-center"
          ...
          <video
            aria-hidden
            className="pointer-events-none block lg:hidden absolute inset-0 h-full w-full object-cover object-center"
        ...
        {/* Chồng thêm lên mép hero để hết seam / subpixel */}
        <section
          className="relative z-[1] w-full -mt-4 sm:-mt-5 md:-mt-6 pt-6 pb-20 sm:pt-8 md:pt-10 px-10 xl:px-20 xll:px-44"
          style={{
        ...
        <section className="flex bg-white relative w-full flex-col items-center justify-center lg:p-40 p-10 overflow-hidden">
          ...
          <div
            ...
            style={{
              backgroundImage: `url(/halloffame/WS-opacity-20.png)`,
            }}
          />
        ...
        <section className="flex bg-white relative w-full h-[570px] overflow-hidden">
          ...
          <div className="absolute top-0 left-0 w-full h-full my-20">
            {/* Hàng ảnh trên */}
            <div className="marquee-container pre-init">
          ...
          <div className="lg:hidden relative z-10 w-full h-full flex flex-row items-center justify-between 2xl:px-[200px] md:px-[120px] text-center text-white">
            <div className="w-full flex flex-col items-center justify-center">
              ...
              />
              <button
                onClick={() => navigate(DEFAULT_SCHOLARSHIP_TALENT_DETAIL_PATH)}
        ...
        {/* Tem tròn — góc dưới phải toàn bộ nội dung, ngay phía trên footer */}
        <img
          src="/stamp.png"
          alt=""
          className="pointer-events-none select-none absolute bottom-0 right-0 w-[clamp(216px,33vw,420px)] h-auto z-[5]"
        />
      </div>
      {/* Footer */}
